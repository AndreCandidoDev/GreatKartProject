...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # messages.success(request, 'Verification email sent to your registered email')
...
def forgotpassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            user = Account.objects.get(email__exact=email)  # da get() e procura email com case sensitive
            current_site = get_current_site(request)
